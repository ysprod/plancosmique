...
const SelectionHeader = memo(() => (
  ...
  >
    ...
    <motion.p
      ...
      className="text-sm sm:text-base text-gray-700 dark:text-gray-300 mb-4 max-w-2xl mx-auto"
    >
      Découvrez les 5 forces qui structurent votre identité cosmique
    </motion.p>

    <ol className="flex flex-col gap-2 items-center mb-5 list-none max-w-md mx-auto">
      {PORTES_LABELS.map((label, idx) => (
        <PorteItem key={label} label={label} index={idx} />
      ))}
    </ol>
    ...
    <motion.p
      ...
      className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 max-w-xl mx-auto 
               leading-relaxed px-4"
    >
      Une étape fondamentale pour comprendre qui vous êtes, comment vous vibrez
      et la manière dont vous vous reliez au monde.
    </motion.p>
  </motion.div>
));
